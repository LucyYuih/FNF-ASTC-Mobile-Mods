...
# Import da biblioteca astc-encoder-py
...
class ASTCConverterGUI:
    def __init__(self, root):
        ...
        # Auto-select images when folder changes
        self.input_folder.trace('w', self.auto_select_images)

        self.build_main_ui()
        # Auto-select on startup if folder exists
        if self.input_folder.get() and os.path.exists(self.input_folder.get()):
            self.auto_select_images()

    def auto_select_images(self, *args):
        """Automatically select images when folder changes"""
        folder = self.input_folder.get()
        if folder and os.path.isdir(folder):
            self.all_images = self.scan_all_images()
            for img in self.all_images:
                w, h = img['size']
                # Select images that are not both dimensions smaller than threshold
            ...
            self.log_message(f"Auto-selected {len(self.selected_images)} images from {folder}")
    ...
    def build_main_ui(self):
        ...
        # Options
        ttk.Label(frame, text="Block Size:").grid(row=1, column=0, sticky="w", pady=5)
        block_combo = ttk.Combobox(frame, textvariable=self.block_size, width=10, values=['Auto'] + BLOCK_SIZES)
        block_combo.grid(row=1, column=1, sticky="w")
        block_combo.bind('<<ComboboxSelected>>', lambda e: self.auto.set(self.block_size.get().lower() == 'auto'))

        ttk.Label(frame, text="Quality:").grid(row=2, column=0, sticky="w", pady=5)
        quality_combo = ttk.Combobox(frame, textvariable=self.quality, width=10,
                                     values=['-fast', '-medium', '-thorough', '-exhaustive'])
        quality_combo.grid(row=2, column=1, sticky="w")

        # Buttons
        ttk.Button(frame, text="Select Images", command=self.open_selection_window).grid(row=3, column=0, pady=10)
        ...
        self.btn_cancel.grid(row=3, column=2, pady=10)

        # Progress and log
        self.progress = ttk.Progressbar(frame, orient="horizontal", mode="determinate")
        self.progress.grid(row=4, column=0, columnspan=3, sticky="we", pady=5)
        ttk.Label(frame, text="Log:").grid(row=5, column=0, sticky="w")
    ...
    def calculate_columns(self, container_width):
        ...
        max_cols = max(1, container_width // min_item_width)
    ...
    def update_image_display(self):
        ...
        try:
            # Use fixed container width for stability
    ...
    def toggle_image_selection(self, img_info):
        """Toggle image selection and update display"""
    ...
    def open_selection_window(self):
        ...
        for img in self.all_images:
            w, h = img['size']
            # Select images larger than minimum size
            img['selected'] = not (w < MIN_SELECT_SIZE[0] and h < MIN_SELECT_SIZE[1])
            img['excluded'] = not img['selected']
    ...
    def start_conversion(self):
        to_convert = self.selected_images  # Use the pre-selected list
    ...
    def convert_one(self, path, callback):
        ...
        try:
            ...
            with Image.open(path) as im:
                w, h = im.size
                im = im.convert('RGBA')
            bx, by = (self.choose_best_block(w, h, size) if self.auto.get() 
                      else tuple(map(int, self.block_size.get().split('x'))))
            im = self.pad_image(im, bx, by)

            # Usar astc-encoder-py para compressão ASTC diretamente
            # Mapear qualidade textual para valor numérico
            q_map = {'-fast': 30, '-medium': 60, '-thorough': 80, '-exhaustive': 100}
            quality_val = q_map.get(self.quality.get(), 60)
            profile = ASTCProfile.LDR  # Perfil LDR linear (como o -cl do astcenc)

            # Compressão via PIL: retorna bytes ASTC
            comp_data = im.tobytes("astc", (profile, float(quality_val), bx, by))
    ...
    def finish_conversion(self):
        ...
        self.btn_cancel.config(state='disabled')
        messagebox.showinfo("Done", "Conversion completed!" if not 
                            self.cancel_event.is_set() else "Cancelled by user.")
